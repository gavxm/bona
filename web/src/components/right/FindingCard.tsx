...
export function FindingCard({ finding, open, related }: { finding: Finding; open: boolean; related: boolean }) {
  ...
  const jumpTo = (tab: string, _field: string) => {
  ...
  return (
    <div
      className={[
        ...
        related && !open ? "border-accent-line shadow-[0_0_0_1px_var(--color-accent-line)]" : "",
      ].join(" ")}
    >
      {/* Head */}
      <div
      ...
      >
        {/* Severity bar */}
        <span className={`shrink-0 w-[3px] self-stretch rounded-full ${colors.text.replace("text-", "bg-")}`} />

        {/* Content */}
        <div className="flex-1 min-w-0">
          <div className="text-[13.5px] font-semibold text-text-primary tracking-tight">{finding.title}</div>
          {finding.detail && (
          ...
          <div className="flex items-center gap-2 mt-2">
            <span className={`font-mono text-[9px] tracking-wide uppercase font-semibold px-1.5 py-0.5 rounded border ${colors.text} ${colors.bg} ${colors.line}`}>
              {finding.severity}
            </span>
            <span className="font-mono text-[9.5px] text-text-muted">{finding.id}</span>
          </div>
        </div>

        {/* Chevron */}
        <span className={`shrink-0 text-text-muted mt-0.5 transition-transform duration-200 ${open ? "rotate-180" : ""}`}>
      ...
      {open && (
        <div className="px-3.5 pb-3.5 pl-7">
          {/* Description */}
          <div className="mt-1">
            <p className="text-[12.5px] leading-relaxed text-text-secondary" style={{ textWrap: "pretty" } as React.CSSProperties}>
              {finding.detail}
            </p>
          </div>
            ...
            <div className="mt-3">
              ...
              <div className="flex flex-wrap gap-1.5">
                {link.centerFields.map((field) => (
                  <span
                    key={field}
                    onClick={(e) => { e.stopPropagation(); jumpTo(link.centerTab, field); }}
          ...
          {finding.reason && (
            <div className="mt-3">
              ...
              <div className={`text-[12.5px] leading-relaxed text-text-primary px-3 py-2.5 rounded-lg border ${colors.bg} ${colors.line}`}>
                {finding.reason}
              </div>
            </div>
          )}
